...
abstract class Etoshore(
    ...
    protected open val ongoingStatusList: Array<String> =
        arrayOf(
            "Publishing",
            "Ativo",
        )
    ...
    override fun pageListParse(document: Document): List<Page> =
        document.select(".chapter-images .chapter-item > img").mapIndexed { index, element ->
    ...
    protected open fun parseSelection(
        document: Document,
        selector: String,
    ): Pair<String, List<Tag>>? {
        ...
        return document.selectFirst(selectorFilter)?.text()?.let { displayName ->
            displayName to
                document.select("#filter-form $selector li").map { element ->
    ...
    open val filterListSelector: List<String> =
        listOf(
            ".filter-genre",
            ".filter-status",
            ".filter-type",
    ...
    protected data class Tag(
        val name: String = "",
        val value: String = "",
        val query: String = "",
    )

    private open class SelectionList(
        displayName: String,
        private val vals: List<Tag>,
        state: Int = 0,
