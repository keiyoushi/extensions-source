...
abstract class MMRCMS
    ...
    constructor(
        ...
        protected open fun parseSearchDirectory(page: Int): MangasPage {
            val manga =
                searchDirectory
                    .subList((page - 1) * 24, min(page * 24, searchDirectory.size))
                    .map {
                        SManga.create().apply {
        ...
        protected val detailStatusOngoing =
            hashSetOf("ongoing", "مستمرة", "en cours", "em lançamento", "prace w toku", "ativo", "em andamento", "activo")
            ...
            scope.launch {
                try {
                    if (supportsAdvancedSearch) {
                        val document =
                            client
                                .newCall(GET("$baseUrl/advanced-search", headers))
                                .await()
                                .asJsoup()
                        ...
                        val document =
                            client
                                .newCall(GET("$baseUrl/$itemPath-list", headers))
                                .await()
                                .asJsoup()
