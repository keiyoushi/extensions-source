...
class MangaTypeFilter(types: List<TagFilterData>) :
...
class StoryStatusFilter(status: List<TagFilterData>) :
    Filter.Group<TagFilter>(
        "حالة القصة",
        status.map { it.toTagFilter() },
    )

class TranslationStatusFilter(tlStatus: List<TagFilterData>) :
    Filter.Group<TagFilter>(
        "حالة الترجمة",
...
class CategoryFilter(categories: List<TagFilterData>) :
    Filter.Group<TagFilter>(
        "التصنيفات",
