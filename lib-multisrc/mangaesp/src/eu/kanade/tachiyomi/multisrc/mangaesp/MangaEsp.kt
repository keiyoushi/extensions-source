...
abstract class MangaEsp(
    ...
    override fun popularMangaParse(response: Response): MangasPage {
        ...
        val topDaily =
            responseData.response.topDaily
                .flatten()
                .map { it.data }
        val topWeekly =
            responseData.response.topWeekly
                .flatten()
                .map { it.data }
        val topMonthly =
            responseData.response.topMonthly
                .flatten()
                .map { it.data }

        val mangas =
            (topDaily + topWeekly + topMonthly)
                .distinctBy { it.slug }
                .additionalParse()
    ...
    protected open fun parseComicsList(
        page: Int,
        query: String,
        ...
        return MangasPage(
            filteredList
                .subList((page - 1) * MANGAS_PER_PAGE, min(page * MANGAS_PER_PAGE, filteredList.size))
                .map { it.toSManga(seriesPath) },
            hasNextPage,
        )
    }
    ...
    private class StatusFilter(
        title: String,
        statusList: Array<Pair<String, Int>>,
    ) : UriPartFilter(
            title,
            statusList,
        )
