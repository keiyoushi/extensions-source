...
abstract class Senkuro(
    ...
    private fun fetchTachiyomiSearchFilters(pageRequest: Int) {
        ...
        if (pageRequest == 1) {
            ...
            genresList =
                filterDto.genres
                    .filterNot { name == "Senkuro" && senkuroExcludeGenres.contains(it.slug) }
                    .map { genre ->
                        FilterersTri(
                            genre.titles
                                .find { it.lang == "RU" }!!
    ...
    private class FilterersTri(
        name: String,
        val slug: String,
    ) : Filter.TriState(name)

    private class GenreList(
        genres: List<FilterersTri>,
    ) : Filter.Group<FilterersTri>("Жанры", genres)

    private class TagList(
        tags: List<FilterersTri>,
    ) : Filter.Group<FilterersTri>("Тэги", tags)

    private class TypeList(
        types: List<FilterersTri>,
    ) : Filter.Group<FilterersTri>("Тип", types)

    private class FormatList(
        formats: List<FilterersTri>,
    ) : Filter.Group<FilterersTri>("Формат", formats)

    private class StatList(
        status: List<FilterersTri>,
    ) : Filter.Group<FilterersTri>("Статус", status)

    private class StatTranslateList(
        tstatus: List<FilterersTri>,
    ) : Filter.Group<FilterersTri>("Статус перевода", tstatus)

    private class AgeList(
        ages: List<FilterersTri>,
    ) : Filter.Group<FilterersTri>("Возрастное ограничение", ages)
