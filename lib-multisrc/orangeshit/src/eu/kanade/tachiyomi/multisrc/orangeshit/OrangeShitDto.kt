...
import eu.kanade.tachiyomi.multisrc.orangeshit.OrangeShit.Companion.CDN_URL
    ...
    val sManga = SManga.create().apply {
        ...
        thumbnail_url = imagem?.let {
            when {
                ...
                else -> "$CDN_URL/obras/${this@toSManga.id}/$it?v=3"
            }
        }
        initialized = true
...
fun MediocreChapterSimpleDto.toSChapter(): SChapter {
    return SChapter.create().apply {
        ...
        url = "/capitulo/$id"
        date_upload = criado_em?.let { OrangeShitDtoDateParser.parse(it) } ?: 0
    }
}
...
fun MediocreChapterDetailDto.toPageList(): List<Page> {
    ...
    return paginas.mapIndexed { idx, p ->
        val imageUrl = "$CDN_URL/obras/$obraId/capitulos/$capituloNome/${p.src}"
        Page(idx, imageUrl = imageUrl)
    }
}

// Simple date helper wrapper to reuse existing parsing approach
object OrangeShitDtoDateParser {
