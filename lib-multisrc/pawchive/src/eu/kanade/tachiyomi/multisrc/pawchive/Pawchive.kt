...
import eu.kanade.tachiyomi.multisrc.pawchive.PawchiveCreatorDto.Companion.serviceName
...
abstract class Pawchive :
    ...
    override val client = network.client.newBuilder()
        .addInterceptor { chain ->
            ...
            // If the response is a 404 Not Found AND the request was aiming for the full-size file server
            if (response.code == 404 && request.url.toString().startsWith(fileUrl)) {

                // Check if the user has the fallback toggle enabled
                if (preferences.getBoolean(FALLBACK_LOW_RES_IMG, true)) {
                    ...
                    val pathIndex = imageUrl.indexOf('/', 8)

                    // Build the new thumbnail URL
                    val fallbackUrl = buildString {
                        append(imgUrl)
                        append("/thumbnail")
                        append(imageUrl.substring(pathIndex))
                    }

                    // Retry the request seamlessly with the new fallback URL
    ...
    private val avatarUrl = baseUrl

    private val fileUrl = baseUrl.replace("//", "//file.")

    private val imgUrl = baseUrl.replace("//", "//img.")

    private fun String.formatAvatarUrl(): String = removePrefix("https://").replaceBefore('/', avatarUrl)
    ...
    private fun searchMangas(page: Int = 1, title: String = "", filters: FilterList? = null, sortBy: Pair<String, String> = "" to ""): MangasPage {
        ...
        val mangas = run {
            val favorites = if (fav != null) {
                ...
                if (response.isSuccessful) {
                    response.parseAs<List<PawchiveFavoritesDto>>().filterNot { it.service.lowercase() == "discord" }
                } else {
                    response.close()
            ...
            val allCreators = response.parseAs<List<PawchiveCreatorDto>>().filterNot { it.service.lowercase() == "discord" }
            allCreators.filter {
        ...
        val final = sorted.subList(fromIndex, toIndex).map { it.toSManga(avatarUrl) }
    ...
    override fun fetchChapterList(manga: SManga): Observable<List<SChapter>> = Observable.fromCallable {
        PawchivePostDto.dateFormat.timeZone = when (manga.author) {
            "Pixiv Fanbox" -> TimeZone.getTimeZone("GMT+09:00")
