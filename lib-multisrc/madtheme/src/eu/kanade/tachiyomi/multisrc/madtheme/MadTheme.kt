...
abstract class MadTheme(
    ...
    override fun mangaDetailsParse(document: Document): SManga = SManga.create().apply {
        ...
        status =
            when (statusText.lowercase(Locale.ENGLISH)) {
    ...
    override fun chapterListParse(response: Response): List<SChapter> {
        ...
        response.body.let { body ->
            json
                .decodeFromString<JsonObject>(body.string())["message"]
                ?.jsonPrimitive
                ?.content
    ...
    override fun chapterListRequest(manga: SManga): Request = MANGA_ID_REGEX.find(manga.url)?.groupValues?.get(1)?.let { mangaId ->
        val url =
            "$baseUrl/service/backend/chaplist/"
                .toHttpUrl()
                .newBuilder()
    ...
    private fun parseGenres(document: Document): List<Genre>? =
        document.selectFirst(".checkbox-group.genres")?.select(".checkbox-wrapper")?.run {
            firstOrNull()
                ?.selectFirst("input")
                ?.attr("name")
                ?.takeIf { it.isNotEmpty() }
