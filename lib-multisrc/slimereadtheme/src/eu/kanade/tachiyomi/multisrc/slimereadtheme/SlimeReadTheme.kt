...
abstract class SlimeReadTheme(
    ...
    protected open fun getApiUrlFromPage(): String {
        ...
        val scriptUrl =
            document.selectFirst("script[src*=pages/_app]")?.attr("abs:src")
        ...
        val apiUrl =
            FUNCTION_REGEX.find(script)?.let { result ->
    ...
    override fun popularMangaRequest(page: Int): Request {
        val url =
            "$apiUrl/book_search?order=1&status=0"
                .toHttpUrl()
                .newBuilder()
     ...
     */
    private fun pageableOf(
        page: Int,
        cache: MangasPage,
    ) = Observable.just(cache).map { mangaPage ->
    ...
    companion object {
        ...
        val FUNCTION_REGEX =
            """(?<script>\[""\.concat\("[^,]+,"\."\)\.concat\((?<infix>[^,]+),":\d+"\)\])""".toRegex(
