...
abstract class GroupLe(
    ...
    override fun searchMangaRequest(
        ...
        val url =
            "$baseUrl/search/advancedResults?offset=${50 * (page - 1)}"
                .toHttpUrl()
                .newBuilder()
        if (query.isNotEmpty()) {
    ...
    override fun mangaDetailsParse(document: Document): SManga {
        ...
        val ratingVotes =
            infoElement
                .select(".col-sm-7 .user-rating meta[itemprop=\"ratingCount\"]")
                .attr("content")
        val ratingStar =
            when {
                ratingValue > 9.5 -> "★★★★★"
        ...
        manga.author = infoElement.select("span.elem_author").first()?.text() ?: infoElement
            .select(
        ...
        manga.genre =
            (
                "$category, $rawAgeStop, " +
                    infoElement
                        .select("p:contains(Жанры:) a, p:contains(Теги:) a")
            ...
            when {
                ...
                infoElement.html().contains("<b>Сингл") -> SManga.COMPLETED
                    ...
                    when (infoElement.selectFirst("span.badge:contains(выпуск)")?.text()) {
                        ...
                        "выпуск завершён" ->
                            if (infoElement.selectFirst("span.badge:contains(переведено)")?.text()?.isNotEmpty() ==
                                true
                            ) {
                                SManga.COMPLETED
                            } else {
    ...
    override fun prepareNewChapter(
        ...
        when {
            ...
            single.containsMatchIn(chapter.name) -> {
                if (chapter.name
                        .substringAfter("Сингл")
                        .trim()
                        .isEmpty()
                ) {
                    chapter.name = DecimalFormat("#,###.##")
                        .format(chapter.chapter_number)
