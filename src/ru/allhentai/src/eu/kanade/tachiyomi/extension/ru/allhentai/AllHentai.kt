...
class AllHentai : GroupLe("AllHentai", "https://20.allhen.online", "ru") {
    ...
    private class GenreList(
        genres: List<Genre>,
    ) : Filter.Group<Genre>("Жанры", genres)

    private class Category(
        categories: List<Genre>,
    ) : Filter.Group<Genre>("Категории", categories)

    private class FiltersList(
        filters: List<Genre>,
    ) : Filter.Group<Genre>("Фильтры", filters)

    private class Tags(
        tags: Array<String>,
    ) : Filter.Select<String>("Тэг (только)", tags)
