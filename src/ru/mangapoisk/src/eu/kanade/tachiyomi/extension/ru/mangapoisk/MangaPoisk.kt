...
class MangaPoisk : ParsedHttpSource() {
    ...
    private fun chapterPageListRequest(
        manga: SManga,
        page: Int,
    ): Request = GET("$baseUrl${manga.url}/chaptersList?page=$page", headers)
        ...
        chapter.date_upload = element.select("span.chapter-date").first()?.text()?.let {
            try {
                when {
                    ...
                    it.contains("дня") || it.contains("дней") ->
                        Date(
                            System.currentTimeMillis() - it.split("\\s".toRegex())[0].toLong() * 24 * 60 * 60 * 1000,
                        ).time
