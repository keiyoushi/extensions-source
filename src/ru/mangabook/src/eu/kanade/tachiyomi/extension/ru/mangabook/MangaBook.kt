...
class MangaBook : ParsedHttpSource() {
    ...
    override fun mangaDetailsParse(document: Document): SManga {
        ...
        manga.genre =
            infoElement
                .select(".vis:contains(Категории) > a")
                .map { it.text() }
                .plusElement(category)
                .joinToString { it.trim() }
        val ratingValue =
            infoElement
                .select(".rating")
                .text()
                .substringAfter("Рейтинг ")
                .substringBefore("/")
                .toFloat() * 2
        val ratingVotes =
            infoElement
    ...
    override fun pageListParse(document: Document): List<Page> =
        document.select(".reader-images img.img-responsive:not(.scan-page)").mapIndexed { i, img ->
    ...
    private class FormatList(
        formas: List<CheckFilter>,
    ) : Filter.Group<CheckFilter>("Тип", formas)
    ...
    ) : Filter.Select<String>("Категории", categories)
