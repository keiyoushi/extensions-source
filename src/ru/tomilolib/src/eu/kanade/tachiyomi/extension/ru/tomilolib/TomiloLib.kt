...
class TomiloLib :
    ...
    override val name = "Tomilo-lib"
    override val baseUrl = "https://tomilo-lib.ru"
    ...
    override fun pageListRequest(chapter: SChapter): Request = GET("$apiUrl${chapter.url}", apiHeaders)
    ...
    private fun SManga.titleId(): String = url.substringAfter("?id=", "").ifBlank { url.substringAfterLast('/').substringBefore('?') }

    private fun TitleDto.toSManga(): SManga = SManga.create().apply {
        url = "/titles/$slug?id=$id"
        title = name.trim()
