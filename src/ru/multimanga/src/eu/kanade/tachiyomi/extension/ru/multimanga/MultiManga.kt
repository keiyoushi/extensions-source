...
class MultiManga : ParsedHttpSource() {
    ...
    override fun searchMangaRequest(
        ...
        val genres =
            filters
                .filterIsInstance<GenreListFilter>()
                .first()
                .state
                .filter { it.state }
                .map { it.id }
        ...
        val url =
            if (query.isNotBlank() || genres.isEmpty()) {
    ...
    override fun chapterFromElement(element: Element): SChapter =
        SChapter.create().apply {
            ...
            date_upload =
                element
                    .select("div:contains(Загружено) time")
                    .attr("datetime")
    ...
    override fun pageListParse(document: Document): List<Page> =
        document.select("a.gallerythumb img").mapIndexed { idx, img ->
