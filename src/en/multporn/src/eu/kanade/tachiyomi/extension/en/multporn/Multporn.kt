...
class Multporn : ParsedHttpSource() {
    ...
    open class URISelectFilter(
        name: String,
        open val filters: List<URIFilter>,
        state: Int = 0,
    ...
    open class TypeSelectFilter(
        name: String,
        filters: List<URIFilter>,
    ) : URISelectFilter(name, filters)

    private class PopularTypeSelectFilter(
        filters: List<URIFilter>,
    ) : TypeSelectFilter("Popular Type", filters)

    private class LatestTypeSelectFilter(
        filters: List<URIFilter>,
    ) : TypeSelectFilter("Latest Type", filters)

    private class SearchTypeSelectFilter(
        filters: List<URIFilter>,
    ) : TypeSelectFilter("Search Type", filters)

    open class TextSearchFilter(
        name: String,
        val uri: String,
    ) : Filter.Text(name) {
    ...
    private class SortBySelectFilter(
        override val filters: List<RequestTypeURIFilter>,
        state: Int,
    ) : URISelectFilter(
            "Sort By",
            filters.map { filter ->
    ...
    private class SortOrderSelectFilter(
        filters: List<URIFilter>,
