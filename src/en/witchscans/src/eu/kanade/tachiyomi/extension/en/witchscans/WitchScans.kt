...
abstract class WitchScans :
    ...
    override fun setupPreferenceScreen(screen: PreferenceScreen) {
        ...
        SwitchPreferenceCompat(screen.context).apply {
            key = SHOW_TAGS_PREF
            title = "Show tags on series page"
            summary = "Display tags (Office Romance, Salaryman, etc.) in the description"
    ...
    override suspend fun fetchMangaUpdate(
        ...
        var updatedManga = if (fetchDetails) document?.series?.toSManga(baseUrl, manga) ?: manga else manga
        if (fetchDetails && preferences.getBoolean(SHOW_TAGS_PREF, true)) {
            val tags = document?.series?.tags
            if (!tags.isNullOrEmpty()) {
                val existing = updatedManga.genre.orEmpty()
                val tagNames = tags.joinToString(", ") { it.displayName }
                updatedManga.genre = if (existing.isBlank()) tagNames else "$existing, $tagNames"
            }
        }
