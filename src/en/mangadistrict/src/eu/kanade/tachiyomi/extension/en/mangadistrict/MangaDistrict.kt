...
class MangaDistrict :
    ...
    override fun mangaDetailsParse(document: Document): SManga {
        val tags =
            document.select(mangaDetailsSelectorTag).mapNotNull { element ->
                element.ownText() to
                    element
                        .attr("href")
                        .removeSuffix("/")
    ...
    override fun fetchSearchManga(
        ...
        if (tagFilter != null && tagFilter.state > 0) {
            ...
            return client
                .newCall(GET(urlBuilder.build(), headers))
                .asObservableSuccess()
                .map { response ->
    ...
    private fun loadTagListFromPreferences(): Set<Pair<String, String>> = preferences
        .getStringSet(TAG_LIST_PREF, emptySet())
        ?.mapNotNull {
            it
                .split('|')
                .let { splits ->
