...
public class Core {
    public static void HCaCha20(
        ...
        for (i = 0; i < 10; i++) {
            ...
            x0 = result[0]; x4 = result[1]; x8 = result[2]; x12 = result[3];
            
            result = quarterround(x1, x5, x9, x13);
            x1 = result[0]; x5 = result[1]; x9 = result[2]; x13 = result[3];
            
            result = quarterround(x2, x6, x10, x14);
            x2 = result[0]; x6 = result[1]; x10 = result[2]; x14 = result[3];
            
            result = quarterround(x3, x7, x11, x15);
            x3 = result[0]; x7 = result[1]; x11 = result[2]; x15 = result[3];
            
            // Diagonal rounds
            result = quarterround(x0, x5, x10, x15);
            x0 = result[0]; x5 = result[1]; x10 = result[2]; x15 = result[3];
            
            result = quarterround(x1, x6, x11, x12);
            x1 = result[0]; x6 = result[1]; x11 = result[2]; x12 = result[3];
            
            result = quarterround(x2, x7, x8, x13);
            x2 = result[0]; x7 = result[1]; x8 = result[2]; x13 = result[3];
            
            result = quarterround(x3, x4, x9, x14);
            x3 = result[0]; x4 = result[1]; x9 = result[2]; x14 = result[3];
        }
