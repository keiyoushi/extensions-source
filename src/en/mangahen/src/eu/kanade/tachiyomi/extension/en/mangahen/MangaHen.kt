...
import eu.kanade.tachiyomi.extension.all.mangahen.SortFilter
import eu.kanade.tachiyomi.extension.all.mangahen.TextFilter
import eu.kanade.tachiyomi.extension.all.mangahen.TypeFilter
import eu.kanade.tachiyomi.extension.all.mangahen.getFilters
...
class MangaHen : HttpSource() {
    ...
    override fun popularMangaParse(response: Response): MangasPage {
        ...
        val hasNextPage = doc.select("a[href*=page]").any { it.text().isEmpty() || it.text().isBlank() }
    ...
    private fun tagSearch(tag: String): String? {
                        ...
                        if (it.state.isNotEmpty()) {
                            it.state.split(",").filter(String::isNotBlank).map { tag ->
                                ...
                                if (trimmed.startsWith('-')) {
                                    tagSearch(trimmed.removePrefix("-"))?.let { tagInfo ->
                                        excludeTags.add(tagInfo)
                                    }
                                } else {
                                    tagSearch(trimmed)?.let { tagInfo ->
