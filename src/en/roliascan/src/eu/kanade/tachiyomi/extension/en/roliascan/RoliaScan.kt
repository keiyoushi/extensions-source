...
class RoliaScan : ParsedHttpSource() {
    ...
    override fun fetchChapterList(manga: SManga): Observable<List<SChapter>> {
        ...
        do {
            ...
            val chapterPage =
                client
                    .newCall(POST(url, headers, formBuilder.build()))
                    .execute()
                    .asJsoup()
    ...
    override fun pageListParse(document: Document): List<Page> =
        document.select(".manga-child-the-content img").mapIndexed { index, element ->
    ...
    private fun getFilters(response: Response) {
        ...
        val script =
            document.selectFirst("script:containsData(FWP_JSON)")?.data()
                ?: return

        script.getDocumentFragmentFilter(buildRegex("genres"))?.let {
            genreList =
                it.select(".facetwp-checkbox").map { element ->
                    Genre(
    ...
    private fun getOptionList(
        ...
        return content
            .getDocumentFragmentFilter(pattern)
            ?.select(cssQuery)
            ?.map { element ->
                Option(
                    name = element.text(),
