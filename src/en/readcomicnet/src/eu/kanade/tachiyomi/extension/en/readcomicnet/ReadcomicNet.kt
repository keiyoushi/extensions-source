...
class ReadcomicNet : ParsedHttpSource() {
    ...
    override fun popularMangaRequest(page: Int): Request = GET("$baseUrl/popular-comic/$page", headers)

    override fun latestUpdatesRequest(page: Int): Request = GET("$baseUrl/comic-updates/$page", headers)
    ...
    override fun mangaDetailsParse(document: Document): SManga = SManga.create().apply {
        ...
        status =
            infoElement
                .selectFirst("td:contains(Status:)+ td")
                ?.text()
                .orEmpty()
                .let { parseStatus(it) }
    ...
    override fun chapterFromElement(element: Element): SChapter = SChapter.create().apply {
        ...
        try {
            date_upload =
                element.nextElementSibling()!!.text().let {
    ...
    override fun pageListParse(document: Document): List<Page> = document.select(".chapter_img").mapIndexed { idx, img ->
    ...
    private class StatusFilter : Filter.Select<String>("Status", arrayOf("", "Complete", "On Going"), 0) {
        ...
        fun stateAsQueryString(): String = stateArray[this.state]
    }
