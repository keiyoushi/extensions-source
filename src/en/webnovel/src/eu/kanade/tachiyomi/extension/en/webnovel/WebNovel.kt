...
class WebNovel : HttpSource() {
    ...
    override fun searchMangaRequest(
        ...
        if (query.isNotBlank()) {
            val url =
                "$baseApiUrl$QUERY_SEARCH_PATH?type=manga&pageIndex=$page"
                    .toHttpUrl()
                    .newBuilder()
    ...
    override fun mangaDetailsParse(response: Response): SManga =
        response.parseAsForWebNovel<ComicDetailInfoResponse>().toSManga(::getCoverUrl)
    ...
    override fun chapterListParse(response: Response): List<SChapter> {
        ...
        val accurateUpdateTimes =
            runCatching {
                client
                    .newCall(GET("$WEBNOVEL_UPLOAD_TIME/${comic.id}.json"))
                    .execute()
        ...
        return filteredChapters
            .zip(updateTimes) { chapter, updateTime ->
    ...
    private fun getCoverUrl(
        comicId: String,
        coverUpdatedAt: Long,
    ): String = "$baseCoverURl/bookcover/$comicId?imageId=$coverUpdatedAt&imageMogr2/thumbnail/1024x"
