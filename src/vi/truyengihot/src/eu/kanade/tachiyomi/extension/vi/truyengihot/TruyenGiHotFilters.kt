...
internal open class GenreGroup(
    name: String,
    private val key: String,
    state: List<Genre>,
) : Filter.Group<Genre>(name, state),
    UriFilter {
    override fun addToUri(builder: HttpUrl.Builder) {
...
internal class PublicationTypeFilter :
    UriPartFilter(
        "Thể loại",
        "genre_add",
        arrayOf(
...
internal class FormatTypeFilter :
    UriPartFilter(
        "Format",
        "format_add",
        arrayOf(
...
internal class ScanlatorFilter(
    vals: Array<Pair<String, String>>,
) : UriPartFilter("Nhóm dịch", "group_add", vals)

internal class TagFilter(
    state: List<Genre>,
) : GenreGroup("Tags", "tag", state)

internal class ThemesFilter(
    state: List<Genre>,
