...
class RaijinScans :
    ...
    protected open val hasPremiumChapters: Boolean = true
    ...
    private val readerManifestRegex =
        Regex("""window\.raijinFreeReaderManifests\.push\s*\(\s*([\s\S]*?)\s*\)\s*;?""", RegexOption.DOT_MATCHES_ALL)
    ...
    override fun pageListParse(response: Response): List<Page> {
        ...
        val isPremium =
            document.select(".subscription-required-message").isNotEmpty() || response.request.url.toString().contains("connexion")
        if (isPremium) {
            throw Exception("This chapter is premium. Please connect via the webview to view.")
        }

        val html = document.outerHtml()
        val content = readerManifestRegex.find(html)?.groups?.get(1)?.value ?: return listOf()
