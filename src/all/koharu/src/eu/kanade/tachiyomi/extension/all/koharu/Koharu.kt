...
class Koharu(
    ...
    private fun getImagesByMangaEntry(entry: MangaEntry): Pair<ImagesInfo, String> {
        ...
        val imagesResponse =
            client
                .newCall(
                    GET(
                        "$apiBooksUrl/data/${entry.id}/${entry.public_key}/$id/$public_key?v=${entry.updated_at ?: entry.created_at}&w=$realQuality",
                        lazyHeaders,
                    ),
                ).execute()
    ...
    override fun mangaDetailsParse(response: Response): SManga = response.parseAs<MangaEntry>().toSManga()
    ...
    override fun pageListRequest(chapter: SChapter): Request = GET("$apiBooksUrl/detail/${chapter.url}", lazyHeaders)
