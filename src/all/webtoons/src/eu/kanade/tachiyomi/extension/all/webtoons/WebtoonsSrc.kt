...
open class WebtoonsSrc(
    ...
    override fun pageListParse(document: Document): List<Page> {
        var pages = document.select("div#_imageList > img").mapIndexed { i, element ->
            val imageUrl = element.attr("data-url")

            if (useMaxQualityPref()) {
                val httpUrl = imageUrl.toHttpUrl()

                // Remove the compression parameter from the URL
                if (httpUrl.queryParameter("type") == "q90") {
                    val newHttpUrl = httpUrl.newBuilder().apply {
                        removeAllQueryParameters("type")
                    }.build()

                    return@mapIndexed Page(i, "", newHttpUrl.toString())
                }
            }
