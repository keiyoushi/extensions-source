...
abstract class Galaxy(
    ...
    override fun latestUpdatesRequest(page: Int): Request {
        val url =
            "$baseUrl/latest?serie_type=webtoon&main_genres=romance" +
                if (page > 1) {
                    "&page=$page"
                } else {
                    ""
    ...
    override fun latestUpdatesParse(response: Response): MangasPage {
        ...
        val entries =
            document.select("div[wire:snapshot*=App\\\\Models\\\\Serie]").map { element ->
                SManga.create().apply {
            ...
            if (filters.isEmpty()) {
                ...
                val mainGenre =
                    FilterData(
                        ...
                        options =
                            document.select("select[wire:model.live=main_genres] option").map {
                ...
                val typeFilter =
                    FilterData(
                        ...
                        options =
                            document.select("select[wire:model.live=type] option").map {
                                it.text() to it.attr("value")
                            },
                        queryParameter = "type",
                    )
                val statusFilter =
                    FilterData(
                        ...
                        options =
                            document.select("select[wire:model.live=status] option").map {
                                it.text() to it.attr("value")
                            },
                ...
                val genreFilter =
                    FilterData(
                        ...
                        options =
                            document.select("div[x-data*=genre] > div").map {
    ...
    override fun mangaDetailsParse(response: Response): SManga {
        ...
        return SManga.create().apply {
            ...
            status =
                when (document.getQueryParam("status")) {
    ...
    override fun chapterListParse(response: Response): List<SChapter> {
        ...
        return document.select("a[href*=/read/]:not([type=button])").map { element ->
            SChapter.create().apply {
                ...
                date_upload =
                    element
                        .selectFirst("div:not(:has(> svg)) > span.text-xs")
                        ?.text()
                        .parseRelativeDate()
            }
        }
