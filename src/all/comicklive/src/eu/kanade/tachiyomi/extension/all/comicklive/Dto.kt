...
import eu.kanade.tachiyomi.extension.all.comicklive.Comick.Companion.GROUP_TAGS_DEFAULT
import eu.kanade.tachiyomi.extension.all.comicklive.Comick.Companion.INCLUDE_MU_TAGS_DEFAULT
import eu.kanade.tachiyomi.extension.all.comicklive.Comick.Companion.SCORE_POSITION_DEFAULT
import eu.kanade.tachiyomi.extension.all.comicklive.Comick.Companion.SHOW_ALTERNATIVE_TITLES_DEFAULT
...
class SearchManga(
    ...
    @SerialName("cdn_id") val cdnId: Int? = null,
    ...
    fun toSManga() = SManga.create().apply {
        // use slug when available, fall back to hid (append # for migration compatibility)
        url = "/comic/${slug ?: hid}#"
        title = this@SearchManga.title
        thumbnail_url = defaultThumbnail ?: parseCover(cover, mdCovers)
    }
}
...
class Manga(
    ...
    fun toSManga(
        ...
        return SManga.create().apply {
            url = "/comic/${comic.slug ?: comic.hid}"
            title = entryTitle
