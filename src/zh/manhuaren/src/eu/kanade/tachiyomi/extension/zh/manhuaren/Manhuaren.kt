...
class Manhuaren :
            ...
            filters.forEach { filter ->
                when (filter) {
                    ...
                    is CategoryFilter -> {
                        url =
                            url
                                .setQueryParameter("subCategoryId", filter.getId())
    ...
    private fun getChapterName(
        ...
        name: String,
        title: String,
    ): String = (if (type == "mangaEpisode") "[番外] " else "") + name + (if (title == "") "" else ": $title")
    ...
    override fun setupPreferenceScreen(screen: PreferenceScreen) {
        SimpleEditTextPreference(screen.context)
            .apply {
                key = USER_ID_PREF
                title = "用户ID"
