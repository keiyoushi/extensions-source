...
class ArabsHentai : ParsedHttpSource() {
    ...
    // ============================== Chapters ==============================
    // تعديل هنا ليشمل كل الروابط الجديدة والقديمة للفصول
    override fun chapterListSelector() = "#chapter-list a[href^='/manga/']"

    override fun chapterFromElement(element: Element) =
        SChapter.create().apply {
            val url = element.attr("href")
            // اسم الفصل من نص الرابط أو من العنصر النصي حسب توفره
            name = element.select(".chapternum").text().ifEmpty { url.substringAfterLast("/").replace("_", " - ").takeIf { it.isNotBlank() } ?: "فصل" }
