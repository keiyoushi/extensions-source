...
abstract class MangaDar : HttpSource() {
    ...
    override val name = "MangaDar"
    override val lang = "ar"
    override val baseUrl = "https://mangadar.com"
    ...
    override fun chapterListParse(response: Response): List<SChapter> {
        ...
        return doc.select("a[href*=/manga/]").mapNotNull { link ->
            val href = link.attr("href")
            val chapterNumber = Regex("/manga/[^/]+/(\\d+)/?$").find(href)?.groupValues?.get(1)
                ?: return@mapNotNull null

            SChapter.create().apply {
                name = "الفصل $chapterNumber"
