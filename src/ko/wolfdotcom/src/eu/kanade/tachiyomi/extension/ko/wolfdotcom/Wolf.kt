...
open class Wolf(
    ...
    private fun domainNumberInterceptor(chain: Interceptor.Chain): Response {
        ...
        if (url.contains(domainRegex)) {
            ...
            val newUrl =
                document.selectFirst("""#pop-content a[href~=^https?://wfwf\d+\.com]""")
                    ?: return response

            response.close()

            val newDomainNum =
                domainRegex.find(newUrl.attr("href"))?.groupValues?.get(1)
