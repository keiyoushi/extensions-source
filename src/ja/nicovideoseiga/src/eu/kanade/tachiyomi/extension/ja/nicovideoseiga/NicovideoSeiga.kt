...
class NicovideoSeiga : HttpSource() {
    ...
    override fun searchMangaRequest(
        ...
        query: String,
        filters: FilterList,
    ): Request = GET("$apiUrl/contents?mode=keyword&sort=score&q=$query&limit=20&offset=${(page - 1) * 20}", headers)
        ...
        return r.data.result
            ...
            .map { chapter ->
                SChapter.create().apply {
                    val prefix =
                        when (chapter.ownership.sellStatus) {
                            "selling" -> "\uD83D\uDCB4 "
                            "pre_selling" -> "\u23F3\uD83D\uDCB4 "
                            else -> ""
                        }
     ...
     */
    private fun getImageType(image: ByteArray): String = if (image[0].toInt() == -1 &&
        image[1].toInt() == -40 &&
        image[image.size - 2].toInt() == -1 &&
        image[image.size - 1].toInt() == -39
    ) {
        "jpeg"
