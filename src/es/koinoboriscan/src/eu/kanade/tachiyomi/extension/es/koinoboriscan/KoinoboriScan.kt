...
class KoinoboriScan : HttpSource() {
    ...
    private val dateFormat =
        SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSS'Z'", Locale("es")).apply {
    ...
    override fun popularMangaParse(response: Response): MangasPage {
        ...
        val mangas =
            (result.mensualRes + result.weekRes + result.dayRes)
                .distinctBy { it.slug }
                .map { it.toSManga() }
    ...
    override fun latestUpdatesParse(response: Response): MangasPage {
        val mangas =
            json
                .decodeFromString<List<SeriesDto>>(response.body.string())
                .map { it.toSManga() }
    ...
    override fun mangaDetailsParse(response: Response): SManga {
        ...
        val jsonData =
            MANGA_DETAILS_REGEX.find(scriptsData)?.groupValues?.get(1)
