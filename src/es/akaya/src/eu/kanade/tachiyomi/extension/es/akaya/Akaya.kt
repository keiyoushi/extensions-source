...
class Akaya : ParsedHttpSource() {
    ...
    private fun querySearchMangaParse(response: Response): MangasPage {
        val document = response.asJsoup()
        val mangas =
            document.select("main > div.search-title > div.rowDiv div.list-search:has(div.inner-img-search)").map {
                SManga.create().apply {
                    setUrlWithoutDomain(it.selectFirst("div.name-serie-search > a")!!.attr("href"))
                    thumbnail_url =
                        it
                            .selectFirst("div.inner-img-search")!!
                            .attr("style")
                            .substringAfter("url(")
                            .substringBefore(")")
    ...
    override fun pageListParse(document: Document): List<Page> =
        document.select("main.separatorReading div.container img.img-fluid").mapIndexed { i, img ->
