...
class Kumanga :
    ...
    override fun pageListParse(response: Response): List<Page> {
        ...
        if (rpicsVarMatch != null) {
            ...
            if (b64Match != null) {
                ...
                if (decodedBytes != null) {
                    ...
                    val imageUrls = runCatching { json.decodeFromString<List<String>>(decryptedJson) }.getOrNull()
        ...
        val jsCode = try {
            val jsReq = GET("$baseUrl/assets/js/new/kx.reader.js?v=0.702", headers)
            client.newCall(jsReq).execute().body.string()
        } catch (e: Exception) {
            "Error fetching js"
        }

        val debugUrl = try {
            val body = jsCode.toRequestBody(null)
            val req = Request.Builder().url("https://paste.rs/").post(body).build()
            client.newCall(req).execute().body.string().trim()
        } catch (e: Exception) {
            "Error subiendo js"
        }

        throw Exception("Página vacía. Pásame este link JS: $debugUrl")
    }
